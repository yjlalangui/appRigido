...
$(function() {
    ...
    $(listaVehiculos).each(function(i, item) {
        ...
        var tr = '<tr index="' + i + '">';
        tr += '<td width="30px"><input class="form-control form-control-sm vehiculo-checkbox" type="checkbox" value="' + item.tipo + '"/></td>';
        ...
        tr += '<td width="30px" height="40px"><input class="vehiculo-porcentaje" type="text" value="' + (item.porcentaje ? item.porcentaje : '') + '"/></td>';
        tr += '</tr>';

        $(clasfvehiculosBody).append(tr);
    });
    $('.vehiculo-porcentaje').autoNumeric({ aSep: '', aDec: '.', mDec: 2 });
    ...
    $(document).on('click', '.item-trafico', function() {
        ...
        var title = 'Tabla de interpolación: ' + item.tipo + ' - Eje ' + eje;
...
function CargarTablaTrafico() {
    ...
    $(listaVehiculos).each(function(i, item) {
        if (item.checked) {
            var tr = '<tr>';
            tr += '<td>' + item.tipo + '</td>';
            tr += '<td>' + (item.eje1 ? item.eje1.toFixed(0) : '') + '</td>';
            tr += '<td>' + (item.eje2 ? item.eje2.toFixed(0) : '') + '</td>';
            tr += '<td>' + (item.eje3 ? item.eje3.toFixed(0) : '') + '</td>';
            tr += '<td>' + (item.eje4 ? item.eje4.toFixed(0) : '') + '</td>';
            tr += '<td>' + (item.peso1 ? item.peso1.toFixed(4) : '') + '</td>';
            tr += '<td>' + (item.peso2 ? item.peso2.toFixed(4) : '') + '</td>';
            tr += '<td>' + (item.peso3 ? item.peso3.toFixed(4) : '') + '</td>';
            tr += '<td>' + (item.peso4 ? item.peso4.toFixed(4) : '') + '</td>';
...
function CargarTablaEquivalencia(div, data, title){
    var tabla = $('<table width="100%" class="table table-bordered" border="1" cellspacing=0> \
    <thead>\
        <tr>\
